"""Database layer - dispatches to PostgreSQL (asyncpg) or SQLite (aiosqlite).

If DATABASE_URL starts with 'postgresql', all public symbols are re-exported
from backend.db_pg (asyncpg-based).  Otherwise the original SQLite
...
if _DATABASE_URL.startswith("postgresql"):
    from backend.db_pg import (  # noqa: F401
        ...
        get_active_barcodes,
        get_barcode_names,
        get_price_full_source_timestamps,
        get_records_by_format_ts,
        count_products,
        count_alerts,
        replace_alerts,
        start_pipeline_run,
        finish_pipeline_run,
        get_pipeline_runs,
        insert_price_full_batch,
        insert_promo_full_batch,
        replace_stores,
        get_raw_table_stats,
    )
    ...
    # All imports and definitions below are only evaluated when NOT using PostgreSQL.
    import json
    import time
    import aiosqlite as _aiosqlite

    # Make aiosqlite accessible as module-level name for type hints used externally
    ...
    async def get_price_full_source_timestamps() -> set:
        async with _aiosqlite.connect(DB_PATH) as db:
            async with db.execute(
    ...
    async def get_records_by_format_ts(pairs: list) -> list:
        if not pairs:
            return []
        async with _aiosqlite.connect(DB_PATH) as db:
            db.row_factory = _aiosqlite.Row
    ...
    async def replace_alerts(alerts: list) -> None:
        now = time.time()
        async with _aiosqlite.connect(DB_PATH) as db:
    ...
    async def get_pipeline_runs(limit: int = 50) -> list:
        async with _aiosqlite.connect(DB_PATH) as db:
            db.row_factory = _aiosqlite.Row
            async with db.execute(
    ...
    async def insert_price_full_batch(records: list) -> int:
        if not records:
            return 0
        now = time.time()
        cutoff = now - 30 * 86400
    ...
    async def insert_promo_full_batch(records: list) -> int:
        if not records:
            return 0
        now = time.time()
        cutoff = now - 30 * 86400
    ...
    async def replace_stores(records: list) -> int:
        if not records:
            return 0
        now = time.time()
        async with _aiosqlite.connect(DB_PATH) as db:
